...
解鎖狀態存在**瀏覽器的 localStorage**。若你在同一台裝置上測試完，要讓對方從頭玩：

- **方式一**：把連結傳給對方，對方用**自己的**手機或電腦開，他的瀏覽器沒有你的進度，會從第 1 題開始。
- **方式二**：在同一台瀏覽器重設：開啟網址時加上 `?reset=birthday_reset`，例如  
  `https://你的網站.onrender.com/?reset=birthday_reset`  
  開一次就會清空進度，網址列會自動變回乾淨的網址，再把**這個乾淨網址**傳給對方即可。  
  若想改用只有你知道的密語，可編輯 `public/js/app.js` 裡的 `RESET_SECRET`。
